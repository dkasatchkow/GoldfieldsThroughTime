...
public class SimpleEnemyAIBehavior1 : MonoBehaviour {
	...
	public float attackRange = 100.0f; //How close does the enemy need to be in order to attack?
	...
	public GameObject bullet;//The bullet or entede the eneme fires
	...
	void Update () {
		...
		if (Vector3.Distance(transform.position,target.transform.position)<attackRange && isfiring == false)
		{
	...
	IEnumerator HuntDownTarget (Vector3 position) {
		...
		while (targetIsOutOfSight) {
			...
			if (TargetIsInSight ()&&(StaticVariables.isSneekDown == false)) {
